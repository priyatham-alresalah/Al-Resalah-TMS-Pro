...
$clientRow = json_decode(
  file_get_contents(
    SUPABASE_URL . "/rest/v1/clients?id=eq.$clientId&select=id,company_name,email,phone",
    false,
    $ctx
  ),
  true
  ...
  <div class="form-card">
    <form method="post" action="../api/portal/update_phone.php">
      ...
      <div class="form-group">
        <label>Company</label>
        <input value="<?= htmlspecialchars($clientRow['company_name'] ?? '') ?>" disabled style="background:#f3f4f6;">
      </div>

      <div class="form-group">
        <label>Email</label>
        <input value="<?= htmlspecialchars($clientRow['email'] ?? '') ?>" disabled style="background:#f3f4f6;">
      </div>

      <div class="form-group">
        <label>Phone</label>
        <input name="phone" value="<?= htmlspecialchars($clientRow['phone'] ?? '') ?>" placeholder="+971..." autocomplete="tel">
      </div>
