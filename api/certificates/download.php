...
ini_set('display_errors', 0); // Don't expose errors to user; use logs
...
if ($certResponse === false) {
  ...
  error_log("Certificate download: Failed to fetch certificate for ID: $id. Error: " . ($error['message'] ?? 'unknown'));
...
if (!$cert) {
  error_log("Certificate download: Certificate not found for ID: $id. Response: " . substr($certResponse, 0, 500));
...
if (empty($cert['candidate_id'])) {
  error_log("Certificate download: Missing candidate_id for certificate ID: $id");
...
if ($candidateResponse === false) {
  ...
  error_log("Certificate download: file_get_contents failed for candidate ID: $candidateId. URL: $candidateUrl. HTTP Code: " . ($httpCode ?? 'unknown') . ". Error: " . ($error['message'] ?? 'unknown'));
...
if (isset($http_response_header) && is_array($http_response_header)) {
  ...
  if ($httpCode && $httpCode >= 400) {
    error_log("Certificate download: HTTP $httpCode when fetching candidate ID: $candidateId. Response: " . substr($candidateResponse, 0, 500));
...
if (!$candidate) {
  error_log("Certificate download: Candidate not found for ID: {$cert['candidate_id']} (Certificate ID: $id)");
...
if ($trainingResponse === false) {
  $error = error_get_last();
  error_log("Certificate download: Failed to fetch training for ID: {$cert['training_id']}. Error: " . ($error['message'] ?? 'unknown'));
...
if (!$training) {
  error_log("Certificate download: Training not found for ID: {$cert['training_id']}. Response: " . substr($trainingResponse, 0, 500));
...
if ($clientResponse === false) {
  $error = error_get_last();
  error_log("Certificate download: Failed to fetch client for ID: {$training['client_id']}. Error: " . ($error['message'] ?? 'unknown'));
...
if (!$client) {
  error_log("Certificate download: Client not found for ID: {$training['client_id']}. Response: " . substr($clientResponse, 0, 500));
...
try {
  ...
  if (!$fileName) {
    error_log("Certificate download: generateCertificatePDF returned null for certificate ID: $id");
    die("Failed to generate certificate PDF");
  }
} catch (Exception $e) {
  error_log("Certificate download: Exception during PDF generation: " . $e->getMessage());
...
exit;
  ...
  error_log("Certificate download.php error: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
